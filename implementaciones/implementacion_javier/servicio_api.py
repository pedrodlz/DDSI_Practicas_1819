...
def get_canciones():
    ...
    for cancion in query_canciones:
        ...
        consulta = "SELECT count(*) FROM escucha WHERE (id_cancion = '"+ cancion[0]+ "' and id_usuario = '"+my_id+"')"
        print(consulta)
        c.execute(consulta)
        cuenta = c.fetchall()
        print(cuenta)
...
def escuchar_cancion():
    ...
    estado=True
    try:
        consulta = "INSERT INTO escucha(id_usuario, id_cancion, fecha) values('"+my_id+"', '"+id+"', to_date('"+hoy+"','DD/MM/YYYY'))"
        c.execute(consulta)
...
def valorar_cancion():
    ...
    estado=200
    try:
        consulta = "INSERT INTO valora(id_usuario, id_cancion, puntuacion) values('"+my_id+"', '"+id+"', "+str(int(valoracion)*2.0)+")"
        c.execute(consulta)
        conn.commit()
    except cx_Oracle.DatabaseError as e:
        error, = e.args
